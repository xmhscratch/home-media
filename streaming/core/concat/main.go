...
func Join(cfg *core.Config, keyId string, concatPaths []string) error {
	...
	go func() {
		...
		reader.WriteVar("Output", filepath.Join(
			...
			core.BuildString(keyId, filepath.Ext(concatPaths[0])),
		))
...
func WriteSegmentFile(cfg *core.Config, keyId string, concatPaths []string) error {
	var (
		err  error
		file *os.File
	)
...
errorLoop:
	for _, filePath := range concatPaths {
		if _, err = file.WriteString(core.BuildString("file ", `'`+filePath+`'`, "\n")); err != nil {
			break errorLoop
		}
	}
