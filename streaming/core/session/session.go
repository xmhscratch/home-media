...
func (T FileSourceType) InitSession(
	...
	var ssCreated int64
	if ssCreated, err = rds.Exists(core.SessionContext, core.BuildString(keyName, ":info")).Result(); err != nil {
	...
	if info, err = rds.HGetAll(core.SessionContext, core.BuildString(keyName, ":info")).Result(); err != nil {
		return nil, true, err
	}
...
func InitDirect(
	...
	if setMode {
		...
		rootId = opts[1:2][0]
		nodeId = opts[2:3][0]
		sessionId = core.GenerateID(core.UUIDNamespace, nodeId)
	}
	...
	if extraInfo, err := ctx.File.InitDirect(); err != nil {
		...
		if err := rds.HSet(
			core.SessionContext,
			core.BuildString(ctx.KeyName, ":info"),
			extraInfo,
		).Err(); err != nil {
			return nil, err
		}
	...
	if setMode || !isCreated {
		if err := rds.HSet(core.SessionContext, core.BuildString(ctx.KeyName, ":info"), map[string]interface{}{
			"rootId":      rootId,
			"nodeId":      nodeId,
...
func InitTorrent(
	...
	if !setMode && isCreated {
		...
		if rawFiles, err = rds.HGetAll(core.SessionContext, core.BuildString(ctx.KeyName, ":files")).Result(); err != nil {
			return nil, err
		}
	...
	if setMode || !isCreated {
		if err := rds.HSet(context.TODO(), core.BuildString(ctx.KeyName, ":info"), map[string]interface{}{
		...
		if err := rds.HSet(
			core.SessionContext,
			core.BuildString(ctx.KeyName, ":files"),
...
func GetFiles(
	...
	if files, err = rds.HGetAll(
		core.SessionContext,
		core.BuildString(keyName, ":files"),
	).Result(); err != nil {
		return nil, err
	}
...
func CreateDownload(
	...
	if result, err := rds.HGet(core.SessionContext, core.BuildString(keyName, ":info"), "sourceType").Result(); err != nil {
		return err
	} else {
